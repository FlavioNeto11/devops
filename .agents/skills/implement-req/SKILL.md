...
**requisito → Codex → PR** (`.github/workflows/req-implement.yml`). A autoria da intenção é no git
(`specs/requirements/**`); aqui a Codex implementa o **código** que satisfaz o requisito. Decisão e
contexto: [`docs/decisions/0002-requirements-as-source-of-truth.md`](../../docs/decisions/0002-requirements-as-source-of-truth.md);
fonte da verdade: [`specs/AGENTS.md`](../../specs/AGENTS.md).
...
- **Blast-radius pelo escopo**: só editar dentro de `allowed_paths` da ordem de trabalho (o produto do
  ...
  `.Codex/**`, `specs/**` (você implementa CÓDIGO, não reescreve o próprio requisito nem a baseline).
...
- **Não mexer em segredos** (herda `.Codex/settings.json`: deny `*.secret.yaml`, `.env`).
...
6. **Encerrar deixando as edições no working tree — SEM git.** A esteira captura (`git add -A`), valida com
   ...
   (status `pr_open`), faz push + `gh pr create` (trailer `Closes-Req` + labels `requirement,Codex-generated`;
...
O trailer `Closes-Req: <REQ-ID>` + label `Codex-generated` ligam PR↔requisito; o
