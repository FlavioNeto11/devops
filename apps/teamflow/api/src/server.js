// server.js — API Fastify (gymops-style). Servida em /teamflow/api (stripPrefix). Gerado pela Forge.
...
const LANDING = landingHtml({ title: 'TeamFlow — Operação de Equipes', base: '/teamflow', stack: 'gymops', caps: ['Fastify', 'Redis/BullMQ', 'RBAC multi-tenant', 'SSO de borda', 'observabilidade'] });
app.addHook('onRequest', async (req) => { const ctx = authContext(req); req.tenantId = ctx.tenantId; req.role = ctx.role; });
app.get('/', async (req, reply) => { if (String(req.headers.accept || '').includes('text/html')) { reply.type('text/html'); return LANDING; } return { app: 'teamflow', service: 'api', ok: true }; });
