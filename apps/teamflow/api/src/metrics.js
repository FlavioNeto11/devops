...
client.collectDefaultMetrics({ register: registry, prefix: 'teamflow_' });
export const M = { recordsTotal: new client.Counter({ name: 'teamflow_records_total', help: 'records', labelNames: ['outcome'], registers: [registry] }),
  jobsTotal: new client.Counter({ name: 'teamflow_jobs_total', help: 'jobs', labelNames: ['status'], registers: [registry] }),
  httpErrors: new client.Counter({ name: 'teamflow_http_errors_total', help: 'erros', registers: [registry] }) };
