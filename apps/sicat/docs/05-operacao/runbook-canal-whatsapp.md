...
| ~~**O1b**~~ | ~~ESCREVER a raia de fila do canal~~ **✅ RESOLVIDA em 2026-08-09.** Deployment `sicat-worker-channel` (`WORKER_LANE=channel`) e `sicat-worker` com `WORKER_LANE=default`, subidos em **dois commits** — a raia nova primeiro, o corte do antigo só depois dela `Ready`, para não haver janela sem consumidor. Mais os Services headless `sicat-worker-metrics` e `sicat-worker-channel-metrics`, que dão alvo ao ServiceMonitor. **Provado em produção**: mensagem injetada com assinatura legítima foi processada pela raia do canal (23,0 s) enquanto a `default` não processou nada | — (era: worker serial único bloqueando `manifest.submit`, e métrica dos workers nunca coletada) |
