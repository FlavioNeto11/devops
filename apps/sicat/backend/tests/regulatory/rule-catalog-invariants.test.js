...
describe('rule-catalog-invariants — meta-guarda 2: evaluator XOR pendência para TODO ruleCode', () => {
  it('RULE_EVALUATORS ∪ RULES_WITHOUT_EVALUATOR_YET = RULE_CODES = codes do seed (26, sem sobreposição, sem órfão)', () => {
    ...
    assert.equal(union.size, 26, `a união deveria cobrir exatamente 26 codes, cobre ${union.size}`);
