// Auth do portal-recorder: leituras são liberadas; escritas exigem
// Authorization: Bearer ${PORTAL_REC_TOKEN}.
...
export function evaluateWriteAuth(authorizationHeader, configuredToken) {
  if (typeof configuredToken !== 'string' || configuredToken.trim().length === 0) {
    return {
      ...
      message: 'PORTAL_REC_TOKEN is not configured; write operations are disabled',
    };
  }
