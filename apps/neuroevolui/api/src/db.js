// db.js — pool Postgres + migrations versionadas (advisory-lock). Gerado pela Forge (gymops-style).
import pg from 'pg';
...
const MIGRATIONS = [
  ...
  `CREATE TABLE IF NOT EXISTS idempotency_keys (idempotency_key TEXT NOT NULL, operation TEXT NOT NULL, entity_type TEXT, entity_id TEXT, response_json JSONB, created_at TIMESTAMPTZ DEFAULT now(), PRIMARY KEY (idempotency_key, operation));`,
];
...
export async function migrate() {
  ...
  try {
    ...
    for (let i = 0; i < MIGRATIONS.length; i++) {
      const v = i + 1; if (done.has(v)) continue;
      await c.query('BEGIN');
