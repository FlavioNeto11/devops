...
app.get('/v1/records', async (req) => ({ data: (await pool.query('SELECT * FROM records WHERE tenant_id=$1 AND deleted_at IS NULL ORDER BY id DESC LIMIT 200', [req.tenantId])).rows }));
app.post('/v1/records', async (req, reply) => { const b = req.body || {}; if (!b.title) { reply.code(400); return { error: { message: 'title obrigatório' } }; } const r = (await pool.query('INSERT INTO records(tenant_id,title,created_by) VALUES ($1,$2,$3) RETURNING *', [req.tenantId, b.title, req.user || 'system'])).rows[0]; M.recordsTotal.inc({ outcome: 'created' }); reply.code(201); return r; });
app.get('/v1/records/:id', async (req, reply) => { const r = (await pool.query('SELECT * FROM records WHERE tenant_id=$1 AND id=$2 AND deleted_at IS NULL', [req.tenantId, Number(req.params.id)])).rows[0]; if (!r) { reply.code(404); return { error: { message: 'não encontrado' } }; } return r; });
app.delete('/v1/records/:id', { preHandler: requireRole('owner') }, async (req) => { await pool.query('UPDATE records SET deleted_at=now(), updated_at=now() WHERE tenant_id=$1 AND id=$2 AND deleted_at IS NULL', [req.tenantId, Number(req.params.id)]); return { deleted: true }; });
