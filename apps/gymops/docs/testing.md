...
- **OPS-001**: 🟡 gate implantado (2026-07-02) — E2E + integração de API rodam em `pull_request` pelo workflow **raiz do monorepo** `.github/workflows/ci-gymops-e2e.yml`. Os workflows aninhados deste app (`ci.yml`/`e2e.yml`) são mortos no monorepo (o GitHub só executa workflows da raiz) e ficam como histórico. A 1ª execução real destravou 3 problemas: BUG-013 (vitest × prom-client — job `integration`, **corrigido**), BUG-014 (`import.spec.ts` ESM×CJS, coleta — **corrigido**) e BUG-015 (seletor de login ambíguo, 35→9 falhas — **corrigido**). Restam **9 testes de smoke desatualizados** (BUG-016) que ainda deixam o job `e2e` vermelho — expectativas divergentes do app real (roteamento por papel, botão de criação por unidade), **não bugs de app** (ver `docs/backlog.md`).
