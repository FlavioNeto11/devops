...
const LANDING = landingHtml({ app: 'opsboard', title: 'OpsBoard — Painel de Operações', base: '/opsboard', stack: 'sicat', caps: ['fila transacional', 'gateway externo', 'idempotência', 'observabilidade'] });
app.get('/', (req, res) => String(req.headers.accept || '').includes('text/html') ? res.type('html').send(LANDING) : res.json({ app: 'opsboard', service: 'api', ok: true }));
...
app.get('/v1/records', wrap(async (req, res) => res.json({ data: await recordsService.listRecords(req.tenantId, req.query.status) })));
