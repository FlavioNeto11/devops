// landing.js — página de status servida na RAIZ do app (para ser navegável no browser, não só API).
// Gerado/padrão da Forge para apps SICAT/gymops sem SPA própria.
export function landingHtml({ app, title, base, stack, caps }) {
  ...
  return [
    ...
    '<div class="links"><a class="p" href="' + base + '/api/health">Ver health da API</a><a href="' + base + '/api/v1/records">Dados (API)</a><a href="/reqs/#/forge">← Forge</a></div>',
    '</div></body></html>',
  ].join('');
}
