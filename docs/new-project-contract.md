...
| `host`      | string | **Sim**     | Host de entrada no Traefik. Local: `xpto.localhost`. Real futuro: `www.xpto.com`.                  | `xpto.localhost`   |
...
- **`host`**: no laboratorio sempre `xpto.localhost`. O mesmo layout de paths vale para o
  dominio real `www.xpto.com` — basta trocar o `host`. Veja
...
#
# Host local: xpto.localhost  | Host real futuro: www.xpto.com
...
> No dominio real, troque `app.host` para `www.xpto.com` (mesmos paths): as URLs viram
> `https://www.xpto.com/aplicacao2`, `.../aplicacao2/api/health`, etc.
