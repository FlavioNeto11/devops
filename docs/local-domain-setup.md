...
o ambiente **local** (`xpto.localhost`, `www.xpto.com`, `traefik.localhost`) e como
preparar o **dominio real futuro** (`www.xpto.com`): DNS (A/CNAME), firewall e port-forward
...
- O navegador precisa **resolver o host** (`xpto.localhost`, `www.xpto.com`) para
...
> entradas no arquivo `hosts` (secao 3). Ja **`www.xpto.com`** NAO e `.localhost` e
...
127.0.0.1 www.xpto.com
127.0.0.1 traefik.localhost
```

- `xpto.localhost` — host unico da plataforma (Console, Argo CD, Grafana, apps).
- `www.xpto.com` — testar **localmente** o layout do dominio real (resolve para o loopback
...
## 5. DOMINIO REAL FUTURO (`www.xpto.com`)

Para servir a plataforma na internet em `www.xpto.com` (mesmo layout de paths do local),
...
|-------|----------------|------------------------------------|-------------------------------------------------------|
| `A`   | `www`          | `<IP_PUBLICO_DA_SUA_CONEXAO>`      | Exposicao direta (IP fixo). Aponta `www.xpto.com` -> IP. |
...
- **Propagacao**: alteracoes de DNS podem levar de minutos a horas. Valide com:
  ```powershell
  Resolve-DnsName www.xpto.com -Server 1.1.1.1
  nslookup www.xpto.com 8.8.8.8
  ```
...
Cloudflare; ela publica `www.xpto.com` com **TLS gerenciado**, **sem** abrir portas no
...
**`C:\Users\<voce>\.cloudflared\config.yml`** mapeando `www.xpto.com` para o Traefik local
...
> roteamento por path); o `Host` header `www.xpto.com` casa as `IngressRoute` do dominio
> real (ajuste o `host` no `devops.yaml`/IngressRoutes para `www.xpto.com`).

### 6.4 Criar o registro DNS do tunel (CNAME automatico)

```powershell
# Cria/atualiza o CNAME www.xpto.com -> <TUNNEL_ID>.cfargotunnel.com na Cloudflare
cloudflared tunnel route dns xpto-local www.xpto.com
```
...
curl.exe -I https://www.xpto.com/devops
```
...
> Lembre-se de ajustar as `IngressRoute`/`devops.yaml` para `Host(www.xpto.com)` — veja a
...
ngrok http --domain=www.xpto.com 80
```
...
(`https://www.xpto.com`). Voce **nao** precisa gerenciar certificados no cluster nem abrir
...
|---------------------------------------------------------|-------------------------------------------------------|--------------------------------------------------------------------------|
| `www.xpto.com` nao abre localmente.                     | Sem entrada no `hosts` apontando p/ `127.0.0.1`.      | Adicione `127.0.0.1 www.xpto.com` (secao 3) e `ipconfig /flushdns`.      |
| `xpto.localhost` nao resolve.                           | Resolvedor nao atende `.localhost` neste host.        | Adicione `127.0.0.1 xpto.localhost` no `hosts`.                          |
| Pagina abre mas vem 404 do Traefik.                     | `Host` nao casa nenhuma `IngressRoute`.               | Confirme o `host` no `devops.yaml`/IngressRoutes (local vs `www.xpto.com`). |
...
| DNS publico nao propagou.                               | TTL/propagacao.                                       | Aguarde; teste com `Resolve-DnsName www.xpto.com -Server 1.1.1.1`.       |
